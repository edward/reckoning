class TimeEntriesController < ApplicationController
  def index
    if params[:filter_class]
      ...
      @days = (@filtered.time_entries.find(:first, :order => 'start_time ASC').start_time.to_date .. (@filtered.time_entries.find(:first, :order => 'start_time DESC').end_time.to_date)).to_a
    else
